...
(case when isnull((select count(*) from OINV where (NumAtCard like '%'+t0.U_DocEntryIM+'%' or  NumAtCard like '%'+t0.U_DocEntryIU +'%') and ((isnull(t0.U_DocEntryIM,'') != '' or isnull(t0.U_DocEntryIU,'') != '' ))),0) > 0then 'FACTURADO' 
